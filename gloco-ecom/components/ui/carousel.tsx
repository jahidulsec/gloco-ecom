...
>(({ className, variant = "outline", size = "icon", ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size={size}
      className={cn(
        "absolute  h-14 w-8 rounded",
        orientation === "horizontal"
          ? "left-4 md:left-4 top-1/2 -translate-y-1/2"
  ...
  return (
    <Button
      ...
      className={cn(
        "absolute h-14 w-8 rounded",
        orientation === "horizontal"
          ? "right-4 md:right-4 top-1/2 -translate-y-1/2"
    ...
    <div className="absolute left-[50%] bottom-5 md:bottom-10 -translate-x-[50%] flex gap-3 items-center">
      {Array.from({ length: count }, (_, i) => i + 1).map((item) => (
        <button
          type="button"
          className={`${
            current === item ? `w-6 md:w-8 bg-secondary` : `w-2 md:w-3 bg-gray-100`
